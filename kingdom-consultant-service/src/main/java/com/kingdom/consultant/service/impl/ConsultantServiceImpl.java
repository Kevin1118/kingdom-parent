...
//            获取所有股票的代码,并获取股票当前价格
    ...
    @Override
    public Map selectProductAndRatio(int consultanId,int productId) {
        Map content=new HashMap();
        Map content1=new HashMap();
        Map content2=new HashMap();
        String name=null;
        Product product=productMapper.selectProductById(productId);
        int fdAmount=product.getFundamount();
        int skAmonut=product.getStockamount();
        ...
        for (int j=0;j<lpfd.size();j++){
                ...
                content1.put(fdCode,fdProportion);
        }
        ...
        for (int j=0;j<lpsk.size();j++){
                ...
                skProportion=lpsk.get(j).getProportion().doubleValue()*skAmonut*0.01;
                content1.put(skCode,skProportion);
        }
    ...
    @Override
    public int assetRebalancing(int consultantId, int productId) {
        List<SignAccount> lsa=userMapper.selectAccountByProductID(productId);
        Map mp=selectProductAndRatio(consultantId,productId);
        int rows=-1;
        int accountNo=0;
        double sum=0;
        for(int i=0;i<lsa.size();i++) {
            ...
            for (int j=0;j<lpy.size();j++) {
                ...
                String type1 = lpy.get(j).getType();
                String type2 = null;
                //如果是股票，去股票备选库查询现值,买入或卖出股票
                if (type1.equals("股票") & amount1>0) {
                    percent= (double) mp.get(code1);
                    valueNow = consultantMapper.selectStockValue(code1).getValueNow().doubleValue();
                    s=amount1*valueNow;
                    double need=sum*percent;
                    int needToBuy=(int)((need-s)/valueNow);
                    amount1+=needToBuy;
                }
                //买入或卖出基金
                if(type1.equals("基金") & amount1>0){
                ...
                rows=propertyMapper.updatePropertyByOrederIdAndCode(orderId,code1,amount1);
            }
        }
        return rows;
    }
